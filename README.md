...
**实测匹配率**：在 4 个广州核心区各 3 公里的采样（750 家唯一门店）中命中 **20/93**。未命中的主因是**采样截断**：每个圆只取到 200 家（按距离排序），番禺、黄埔、白云以及圆圈边缘的店根本不在样本里；线上运行时用环形分片能取到 400 家，实际命中会更高。要进一步提升可给条目补 `amapPoiId`（用高德关键字搜索逐条解析，93 条约消耗 93 次配额）。
