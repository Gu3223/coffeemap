...
function MapView({ places, selected, onSelect, location, accuracy, onLocate }) {  const center = useMemo(() => location ? [location.latitude, location.longitude] : [0, 0], [location?.latitude, location?.longitude])
...
function App() {
  ...
  const toggleFollowing = () => { if (isFollowing) { if (watchId.current != null) navigator.geolocation.clearWatch(watchId.current); watchId.current = null; setIsFollowing(false); return } if (!navigator.geolocation) { clearLocationData('当前浏览器不支持实时定位。'); return } setIsFollowing(true); setLocationStatus('requesting'); watchId.current = navigator.geolocation.watchPosition(applyPosition, () => { if (watchId.current != null) navigator.geolocation.clearWatch(watchId.current); watchId.current = null; setLocationStatus('denied'); setIsFollowing(false); setPlacesError(location ? '实时定位暂时中断，当前保留上一次位置结果。' : '无法获取当前位置，请允许定位后重试。') }, { enableHighAccuracy: true, timeout: 20000, maximumAge: 5000 }) }
