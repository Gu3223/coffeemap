...
async function fetchNearbyCafes(location, signal) {
  if (AMAP_KEY) {
    try {
      const amapCafes = await fetchAmapCafes(location, signal)
      if (amapCafes.length) return amapCafes
    } catch (error) {
...
function App() {
  ...
  const dataSourceLabel = cafes.some(cafe => cafe.source === 'amap') ? '高德地图数据' : 'OpenStreetMap 数据'
