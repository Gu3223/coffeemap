...
**匹配规则**：先按 POI id 精确匹配，失败再按名称匹配。名称匹配只用两种情况：归一化后**完全相等**，或**门店名以策展名开头**（对应分店后缀的写法，如「忍忍咖啡惠吉西路总店」）。**刻意不用子串判断**——实测高德存在归一化后只剩通用词的门店名（`COFFEE(恒润广场店)` 归一化成 `coffee`），子串会把「T12 coffee」错配上去；短名同样危险，「Opia」会命中「UtopiaPortal池」。这两种假命中都有回归测试守着。**匹配失败只是不加分，不会隐藏门店**。

**界面上有三个入口**：卡片上的橙色「口碑推荐」标记、筛选里的「口碑推荐」、排序按钮切到「口碑优先」（距离优先 / 评分优先 / 口碑优先 三态循环）。详情抽屉里显示口碑分、短评与来源。

**当前清单**：93 条广州门店，来源为「小红书笔记汇总（用户提供）」——即用户自己阅读笔记后提供的店名与要点，属于合法来源。`score` 是**策展评分（主观）**，按来源文字的推荐强度给 6-9 分，不是实测数据。

**实测匹配率**：在 4 个广州核心区各 3 公里的采样（750 家唯一门店）中命中 **20/93**。未命中的主因是**采样截断**：每个圆只取到 200 家（按距离排序），番禺、黄埔、白云以及圆圈边缘的店根本不在样本里；线上运行时用环形分片能取到 400 家，实际命中会更高。

**已解析 POI id**：用高德关键字搜索逐条解析了 **64/93** 条（消耗 93 次配额），这些条目的 `amapPoiId` 与 `amapName` 已写回文件。剩下 **29 条未解析**，原因是高德收录的名称与笔记里的叫法差异过大（如「醒CAFE」对应高德可能叫「醒咖啡」），前缀规则不允许自动猜，硬猜会把推荐标到别的店上。这 29 条仍靠名称兜底，匹配不到就只是不加分。

```json
{ "name": "忍忍咖啡", "score": 9, "source": "小红书笔记汇总（用户提供）", "note": "越秀·惠吉西…", "amapPoiId": "B0FFH…", "amapName": "忍忍咖啡(惠吉西路总店)", "amapDistrict": "越秀区" }
```
